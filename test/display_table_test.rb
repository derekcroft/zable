...
class DisplayTableTest < ActionView::TestCase

  ## Test helpers for this gem
  def assert_html_table(collection)
    assert_match /<table.*>.*<\/table>/, display_table(collection, Item, &@COLUMN_PROC)
  end
  ...
  test "modules exist in the current scope" do
    assert_kind_of Module, DisplayTable
    assert_kind_of Module, DisplayTable::Html
    assert_kind_of Module, DisplayTable::Sort
    assert_kind_of Module, DisplayTable::Sort::ActiveRecord
    assert_kind_of Module, DisplayTable::ActiveRecord
    assert_kind_of Module, DisplayTable::ActiveRecord::ClassMethods::Helpers
    assert_kind_of Module, DisplayTable::WillPaginate
    assert_kind_of Module, DisplayTableHelper
  end

  ## test overall functionality of plugin
  test "helper method can be called with an empty collection" do
    collection = []
    assert_nothing_raised { display_table collection, Item, &@COLUMN_PROC }
  end

  # this is important if you want to have columns hidden based on a condition:
  # display_table(@complaints, Complaint) do
  ...
  test "empty_table_body_row called when collection is empty" do
    collection = []
    DisplayTableTest.any_instance.expects(:empty_table_body_row)
    display_table collection, Item, &@COLUMN_PROC
  end

  test "empty table body row creates a tr with a td" do
    columns = []
    @rendered = empty_table_body_row(columns, {})
    assert_select "#display-table-empty-set"
  end
